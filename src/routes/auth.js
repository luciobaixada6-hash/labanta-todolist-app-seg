...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    res.redirect('/login');
    console.log(`Novo utilizador registado: ${username}`);
...
router.post('/login', async (req, res) => {
  ...
  if (user) {
    match = await bcrypt.compare(password, user.password_hash); // Comparação com hash vazio para evitar timing attacks
  }
